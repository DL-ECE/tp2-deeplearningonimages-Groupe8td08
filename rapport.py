...
Original file is located at
    https://colab.research.google.com/drive/1ro6THE_EQSljOIhVGQ9fZQhrf1xeRrQx
...
def build_image_like_tensor(n_rows:int, n_colums: int, n_channels:int, default_value: int)-> np.ndarray:
  ...
  """
  # YOUR CODE HERE
  tensor = np.full((n_rows, n_colums, n_channels),default_value)   # Create an tensor of shape [2,2,2] of all zeros
  return tensor
build_image_like_tensor(2,3,4,2)#tensor test
...
def sigmoid(input_tensor: torch.Tensor) -> torch.Tensor:
    ...
    # YOUR CODE HERE
    math_torch=torch.sigmoid(input_tensor)
    return math_torch

def softmax(input_tensor: torch.Tensor)-> torch.Tensor:
    """Apply a softmax to the input tensor"""
    # YOUR CODE HERE 
    math_torch=torch.softmax(input_tensor,dim=1)
    return math_torch

def target_to_one_hot(targets: torch.Tensor, num_classes=10) -> torch.Tensor:
    """Create the one hot representation of the target""" 
    # YOUR CODE HERE 
    one_hot = torch.zeros([targets.shape[0], num_classes], dtype=torch.float32)
